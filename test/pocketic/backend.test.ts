...
// ---------------------------------------------------------------------------
// Core backend behavior driven through a small number of shared canisters:
// empty-state reads, the photo round-trip, the archive contribution/approval
// flow, account identity, the profile-claim flow, and updateOwnProfile.
//
...
// and CONTRIBUTOR as a regular #user, then makes CONTRIBUTOR an APPROVED family
// member. Family-content contribution endpoints now require approved-family
// membership rather than mere sign-in, so every contribution test must seed an
// approved contributor; the onboarding/claim paths that stay open to unapproved
// users are covered separately in contribution-authorization.characterize.test.ts.
//
// Idempotent: several tests share the `actor` canister and call this more than
// once, so it only requests a claim when CONTRIBUTOR has none yet and only
// approves a claim that is still pending.
